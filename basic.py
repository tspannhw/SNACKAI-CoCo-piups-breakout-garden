...
try:
  while True:
    ...
    lsm303d_accelerometer = "{:+06.2f}g : {:+06.2f}g : {:+06.2f}g".format(*lsm3accl)
    bme680_tempf = float('{0:.2f}'.format((sensor.data.temperature * 1.8) + 32))
    diskusage = "{:.1f} MB".format(float(usage.free) / 1024 / 1024)
    lsm303d_magnetometer = "{:+06.2f} : {:+06.2f} : {:+06.2f}".format(*lsm3mag)
    print(lsm303d_magnetometer)
    print(bme680_tempf)

    with canvas(oled) as draw:
           draw.rectangle(oled.bounding_box, outline="white", fill="black")
           draw.text((0, 0), "- Sensor -", fill="white")
           ...
           draw.text((0, 80), 'TempF: {}'.format( bme680_tempf ), fill="white")
           draw.text((0, 90), 'A: {}'.format(lsm303d_accelerometer), fill="white")
           draw.text((0, 100), 'M: {}'.format(lsm303d_magnetometer), fill="white")
           draw.text((0, 110), 'DU: {}'.format(diskusage), fill="white")
           #time.sleep(0.5)
